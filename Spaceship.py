...
class Ship:
    ...
    def shoot(self):
        ...
        vel = list(self.vel)
        for i in range(2):
            pos[i] += acc[i] * self.image_center[0]
            vel[i] += acc[i] * 5
...
def draw(canvas):
    ...
    #Score and Lives
    canvas.draw_text("Score", (WIDTH*.85, HEIGHT * .04), 22, "White", "sans-serif")
    canvas.draw_text(str(score), (WIDTH*.88, HEIGHT * .10), 22, "White", "sans-serif")
...
def rock_spawner():
    ...
    a_rock.angle_vel = (random.random() - 0.5) * 0.05
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [0, 0], 0, 0.0, asteroid_image, asteroid_info)
